...
for ($pos=0; $pos<count($urls); $pos++)
{
	...
	{ 
		...
		if($rarPartX < $rarPartY || $rarPartY == 0 )  $new_line="<BR>";
...
$imagebam = preg_replace("/\[\/URL\] /i", "[/URL]\n", $imagebam);
//$imagebam = preg_replace("/medium/i", "original", $imagebam);
//$imagebam = preg_replace("/_0\.jpg/i", ".jpg", $imagebam);
$imagebam = explode("\n", $imagebam);
...
var_dump($pimpandhost);
//var_dump($imagebam);
//$imagebam	= preg_replace("/url\] \[url/i", "url]\n[url", $imagebam); 
//$imagebam	= preg_replace("/ /i", "", $imagebam); 
...
//foreach($imagebam as $current_image)$imagebam2[]=trim($current_image);
//$imagebam = insertMissingRows($imagebam);
...
foreach ($new_urls as $url)
{
    ...
    '[/b]<BR><BR>More screenshots:<BR>'.$imagebam[$i].'<BR><BR><BR>","'.
    ...
    $all_posts .= $current_post;
  	...
  	$pimpandhost[$i].'<BR><BR>[b]'.$genre.', '.$tags.'[/b]<BR>'.$titles[$i].'<BR><BR>'.$filesinfo[$i]. 
  	'<BR><BR>[b]~> Download video from KEEP2SHARE <~<BR>'.$url.
  	'[/b]<BR><BR>More screenshots:<BR>'.$imagebam[$i].'<BR><BR><BR><BR><BR><BR><BR><BR><BR>';
...
$datasource=substr(file_get_contents("/arc/scr/upldir"), 0, strlen(file_get_contents("/arc/scr/upldir"))-2);

$posts_bbcode_path = '/root/iMacros/Datasources/' . $datasource . '.csv';
